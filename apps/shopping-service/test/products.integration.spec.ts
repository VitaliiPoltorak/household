...
describe('Products (integration)', () => {
  ...
  it('rejects a product name shorter than 3 characters (400)', async () => {
    await request(app.getHttpServer())
      .post('/products')
      .set('X-Household-Id', H)
      .send({ name: 'ab' })
  ...
  it('filters by preferred store', async () => {
    ...
    await request(app.getHttpServer())
      .post('/products')
      .set('X-Household-Id', H)
      .send({ name: 'Product A', preferredStoreId: storeId });
    await request(app.getHttpServer())
      .post('/products')
      .set('X-Household-Id', H)
      .send({ name: 'Product B', preferredStoreId: storeB });
  ...
  describe('Cross-household reference isolation (#67)', () => {
    ...
    it('rejects POST /products with a foreign store in alternativeStoreIds', async () => {
      ...
      await request(app.getHttpServer())
        ...
        .send({
          name: 'Milk',
          alternativeStoreIds: [storeId, foreignStore.body.id],
        })
        .expect(404);
    });
