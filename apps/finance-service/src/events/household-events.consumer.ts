...
/**
 * Cleans up finance-schema rows when a household is deleted in household-service
 * (issue #83.4). Delivery is at-least-once, so the transaction re-runs safely:
 * a second delete of an already-empty household is a no-op.
 *
    ...
    await this.consumer.subscribe<{ householdId: string }>(
      ...
      async (envelope) => {
        ...
        await this.ds.transaction(async (m) => {
          ...
          const rp = await m.getRepository(RecurringPayment).delete({ householdId });
          const ac = await m.getRepository(Account).delete({ householdId });
          const ct = await m.getRepository(Category).delete({ householdId });
          const is = await m.getRepository(IncomeSource).delete({ householdId });
          this.logger.log(
