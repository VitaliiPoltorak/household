    ...
    <div className="flex h-full gap-6">
      ...
      <div className="flex w-72 shrink-0 flex-col gap-4">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold text-gray-900">{t('shopping.title')}</h1>
        ...
        {isLoading ? (
          ...
          <p className="text-sm text-gray-400 text-center py-8">{t('shopping.emptyLists')}</p>
        ) : (
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400">{purchased}/{total} {t('shopping.purchased')}</p>
        </div>
      ...
      {list.status === 'active' && (
        <form onSubmit={submit} className="flex gap-2">
          ...
          <input
            ...
            className="w-16 rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
      ...
      <div className="divide-y divide-gray-100 rounded-xl border border-gray-200 bg-white">
        {(list.items ?? []).length === 0 ? (
          <p className="py-8 text-center text-sm text-gray-400">No items yet.</p>
        ) : (
          (list.items ?? []).map((item) => (
            <ItemRow
              key={item.id}
...
function ItemRow({
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3">
      ...
      <span className={`flex-1 text-sm ${item.isPurchased ? 'line-through text-gray-400' : 'text-gray-900'}`}>
        {item.name}
        {item.quantity > 1 && <span className="ml-1 text-gray-400">×{item.quantity}</span>}
      </span>
      {isActive && (
        <button onClick={onDelete} className="text-gray-300 hover:text-red-400 text-sm">✕</button>
      )}
    </div>
  );
}
