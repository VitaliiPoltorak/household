...
import { DemoTransactionRow } from '../components/onboarding/DemoRows';
...
export function TransactionsPage() {
  ...
  const { filters, hasFilters, setType, setAccountId, setFrom, setTo, clear } =
  ...
  if (!activeHousehold && !isTourPage('/transactions'))
    return (
    ...
    <div className="space-y-5">
        ...
        >
          {transactions.map((tx) => (
            <TxRow
              ...
              counterAccountName={
                tx.counterAccountId
                  ? (accountMap[tx.counterAccountId]?.name ??
                    tx.counterAccountId)
                  : null
              }
              categoryName={
      ...
      {confirmDelete && (
        <DeleteTxConfirm
          tx={confirmDelete}
          accountName={
            accountMap[confirmDelete.accountId]?.name ?? confirmDelete.accountId
          }
          counterAccountName={
            confirmDelete.counterAccountId
              ? (accountMap[confirmDelete.counterAccountId]?.name ??
                confirmDelete.counterAccountId)
              : null
          }
