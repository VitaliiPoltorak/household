...
Built (#348) — `libs/contracts/src/feature-flags/registry.ts` is the static, in-git source of truth for flag metadata (key, description, status, expiry, rollout issue); `household-service` owns the mutable runtime state (`feature_flags` / `feature_flag_overrides` tables) and exposes `GET /feature-flags` + household-override endpoints; `@household/feature-flags` (new lib) gives a consuming service `@RequireFeature('flag-key')` + a guard backed by a short-TTL Redis cache, invalidated via the `feature-flag.updated` Kafka event. Resolution order: user override → household override → global default → (household-service unreachable) registry default. Flags are resolved server-side only — the client never supplies one. `scripts/feature-flag.js` is the ops kill-switch for the global default (no platform-admin role exists yet, so this is a direct-DB-write script, not a REST endpoint — see #348's design note). Every flag needs an `expires_at` + linked rollout issue (except a small, capped set of `kill-switch` flags, enforced by `libs/contracts/test/registry.spec.ts` in CI) and tests for both the on and off state. #230 (paid-feature gating) will ride the same `feature_flag_overrides` table via `source: 'subscription'` — not built yet. Full design lives in the `feature-flags` skill (auto-loads for feature-flag work).
