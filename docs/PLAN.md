...
✔ Feature-flag infrastructure (#348)
    ...
    ✔ Gate the Monobank integration behind a kill-switch flag — first real consumer (`@RequireFeature('monobank-integration')` on connect/sync/map; reads and disconnect stay open)
```
...
✔ Transactional email provider (#323) — production sends real mail through Resend.
  Local dev is solved separately: `docker-compose.override.yml` (auto-loaded only when COMPOSE_FILE
  is unset, so it never reaches the VPS) runs a Mailpit catcher (SMTP 1025, UI :8025) and points
  auth-service at it, so register -> verify is exercisable through the UI with no external provider.
    ✔ Resend account with `mail.h-holds.com` verified as the sending domain — DKIM at
      `resend._domainkey.mail.h-holds.com`, SPF (`v=spf1 include:amazonses.com ~all`) and the
      bounce MX (`feedback-smtp.eu-west-1.amazonses.com`) on `send.mail.h-holds.com`, plus a
      `_dmarc.h-holds.com` record. Note the sending identity is the *subdomain*, so
      MAIL_FROM is `Household <no-reply@mail.h-holds.com>`, not `@h-holds.com`.
    ✔ SMTP_HOST / SMTP_PORT / SMTP_SECURE / SMTP_USER / SMTP_PASSWORD, MAIL_FROM and
      WEB_APP_URL (`https://app.h-holds.com`) set in /opt/household/.env on the VPS. WEB_APP_URL
      matters on its own: its default builds the account-unlock link against localhost, so without
      it a locked-out user gets a dead link. SMTP_REQUIRE_TLS stays at its default — `false` is
      for the local catcher only.
    ✔ The port, not the provider, was the real blocker. netcup closes outbound 25/465/587 by
      default (anti-spam), so `SMTP_PORT=587` failed with `Connection timeout` while the
      credentials, DNS and code were all already correct — the same symptom as having no
      provider at all, because delivery is best-effort and the UI reports success either way.
      Production runs Resend's alternate `2465` with SMTP_SECURE=true. Documented in README ->
      Deployment -> Outbound mail, with the port probe and the `verify()` one-liner that
      distinguishes a blocked port from bad credentials.
    ✔ auth-service recreated (`up -d --force-recreate` — `restart` does not re-read .env);
      the boot log no longer carries "No SMTP transport configured (SMTP_HOST is empty)", and a
      nodemailer `verify()` from inside the container returns OK, proving TCP + TLS + auth.
    □ A real register -> verify -> signed-in run on app.h-holds.com against a real mailbox.
      Disposable/temp-mail domains are not a valid test target — Resend blocks them, which is
      why the earlier attempts against hebase.com / emailinbo.live / liondapt.com would have
      failed even with the port open.
